    ...
    Plugin.prototype = {
        _init: function () {
            ...
            if (this.options.position === 'top') {
                var btnTop = $(btn).offset().top - $('#emoji_container_' + ix).height() - 5;
                var btnLeft = $(btn).offset().left;
                $('#emoji_container_' + ix).css({'top': btnTop + 'px', 'left': btnLeft + 'px'});
            } else {
                var btnTop = $(btn).offset().top + $(btn).outerHeight() + 5;
                var btnLeft = $(btn).offset().left;
                $('#emoji_container_' + ix).css({'top': btnTop + 'px', 'left': btnLeft + 'px'});
            }

            // 滚动条美化，如果不用滚动条插件，删除这里就行了
            if ($.fn.slimScroll) {
                $('#emoji_container_' + ix + ' .emoji_content').slimScroll({
                    height: 277
                });
            }
